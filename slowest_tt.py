...
if __name__ == "__main__":
    ...
    for id in maps:
        ...
        if wr == 0:
            continue
        tt = maps[id]["tts"][9]
        p = 1000000000.0 # default if guaranteed tt on completion
        if tt != 0:
            p = 100.0 * (tt["duration"] - wr["duration"]) / wr["duration"]
        percentages.append({
            "id": id,
            "name": maps[id]["name"],
            "percentage": p
        })
    percentages.sort(key=lambda p: p["percentage"], reverse=True)
    pprint(percentages)
